...
- Consider the counter being on a remote server and clients doing the
  ...
  happened before or after the server receieved the increment;
...
and crashing threads and works on arbitrary datastrucures. This is what
we use in parallel property-based testing.
...
model. For a more detailed explaination see my [previous
...
`runReal` function, where `ask` retrives the `sig`nal via the reader
monad:
...
2.  Currently random interleavings are checked, but we could also
    ...
    be more inline with what model checkers do. Perhaps
...
3.  While the approach in this post works in all languages due to its
    ...
    - Intercepting and recording the syscalls that the multi-threaded
      ...
      rerun. I believe this is what Mozilla's time traveling debugger,
    ...
    languages can be incrementally changed to expose scheduler hoos or
